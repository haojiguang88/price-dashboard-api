...
router.post("/buying-plans", async (req, res) => {
  try {
    ...
    const result = await db.run(
      "INSERT INTO buying_plans (plan_name, category_name, object_name, variant_name, target_price, plan_quantity, total_amount, note, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
router.put("/buying-plans/:id", async (req, res) => {
  try {
    ...
    if (!existingPlan) {
      return res.status(404).json({ status: "error", message: "买入计划不存在" });
    }
