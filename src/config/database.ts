...
const initDatabase = async (db: Database) => {
  ...
  await db.exec("CREATE TABLE IF NOT EXISTS position_batches (id INTEGER PRIMARY KEY AUTOINCREMENT, source_id TEXT, position_id INTEGER NOT NULL, batch_date TEXT NOT NULL, batch_price REAL NOT NULL, batch_quantity INTEGER NOT NULL, batch_cost REAL NOT NULL, remaining_quantity INTEGER NOT NULL, note TEXT, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (position_id) REFERENCES positions(id) ON DELETE CASCADE)");
